  ...
  teleport(v-if="pickerShow" to="body")
    .date-picker(ref="picker" :style="pickerStyle" @mousedown="preventClose = true")
      ...
      div
        transition(name="fade" @enter="scrollToView($refs)")
          ...
          .time-picker(v-else-if="jumping === 2")
            div(v-for="(list,refIndex) in timeRender" :key="refIndex")
              ...
              rk-scroll(:get-target="()=>$refs.times[refIndex]")
          .month-picker(v-else-if='jumping === 3')
            ol(ref="months")
              li(
                ...
                @click='setMonth(index);jumping = 0'
                @transitionend="jumping = 0"
              ) {{ i.label }}
        .arrow
        ...
        ul.calendar(@wheel="wheelHandler")
          li(
            ...
            :class='day.classList',
            :data-text="day.text"
            @click='day.disabled || setDay(day.value)'
          ) 
      span(:class="{hidden:!showSubmit}")
        ...
        b(:class="{disabled: outOfRange}" @click='setValue()') {{texts["選擇"]}}
</template>
<style lang="sass" scoped>
[data-theme='-1'],[data-theme='1']
  .date-picker
    --picker-body-bg: white
    --picker-footer-bg: #f7f7f7
    --range-bg: #eff1f5
</style>
<style lang="sass" scoped>
.rk-date-input
