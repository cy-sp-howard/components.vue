...
.rk-date-range-input(:class="{selecting,disabled:disabled ===true}")
  .wrapper
    rk-date-input(v-model="_start" :class="pickerClass.start" v-bind="childrenProps[0]" :init-value="initValue && initValue[0]" @open="setSelecting(1)" @close="setSelecting(-1)" @pick="e=>(pickerDayValues.start = e)")
    rk-date-input(v-model="_end" :class="pickerClass.end" v-bind="childrenProps[1]" :init-value="initValue && initValue[1]" @open="setSelecting(1)" @close="setSelecting(-1)" @pick="e=>(pickerDayValues.end = e)")
    ...
    > :deep(.rk-date-input)
      ...
      &.selecting,&:hover
        ...
        &.empty > .rk-input > input
          box-shadow: inset 0 0 20px var(--color-1)
